...
contract MultiItemAuction {
    ...
    constructor(uint _biddingTime, uint[] memory _itemIds, uint[] memory _startingBids) {
        require(_itemIds.length == _startingBids.length, "Item IDs and starting bids arrays must have the same length.");
    ...
    function bid(uint itemId) public payable {
        ...
        require(msg.value >= items[itemId].startingBid, "Bid must be equal or higher than the item's starting bid.");
        require(msg.value > items[itemId].highestBid, "There already is a higher bid.");

        if (items[itemId].highestBid != 0) {
            pendingReturns[itemId][items[itemId].highestBidder] += items[itemId].highestBid;
        }
